...
#ifdef MSW
static void
SelectNewDeleteOld (HGDIOBJ obj)
{
  obj = SelectObject (TreeDrawingAreaDB->hdc, obj);
  DeleteObject (obj);
}
...
RefreshPen (void)
{
  HPEN pen = CreatePen (PS_SOLID, TreeDrawingAreaDB->LineWidth,
			GetTextColor (TreeDrawingAreaDB->hdc));
  SelectNewDeleteOld (pen);
}

BOOL SetNewFont (LPLOGFONT lplf)
{
  HFONT font = CreateFontIndirect (lplf);
  if (font)
    SelectNewDeleteOld (font);
  return font != NULL;
}
#endif
...
{
#ifdef MSW
  *width  = TreeDrawingAreaDB->DrawingWidth;
  *height = TreeDrawingAreaDB->DrawingHeight;
#else
  Dimension w, h;
     ...
     int width, height;
{
  ...
  TreeDrawingAreaDB->DrawingWidth = width;
  TreeDrawingAreaDB->DrawingHeight = height;
  ...
  si.nPage = rc.bottom;
  si.nPos = pt.y;
  SetScrollInfo (hwndClient, SB_VERT, &si, TRUE);
  ...
  si.nPage = rc.right;
  si.nPos = pt.x;
  SetScrollInfo (hwndClient, SB_HORZ, &si, TRUE);
#else
     ...
     char *label;
{
  ...
  while (len > 1) {
#ifdef MSW
    GetTextExtentPoint32 (TreeDrawingAreaDB->hdc, label, len, &sz);
    node->width  = sz.cx + (LABEL_MAT_WIDTH * 2) + 1;
    node->height = sz.cy + (LABEL_MAT_HEIGHT * 2) + 1;
#else
...
#ifdef MSW
  node->label.yoffset = LABEL_MAT_HEIGHT + 1;
#else
...
{
#ifdef MSW
  SetTextColor (TreeDrawingAreaDB->hdc, TreeDrawingAreaDB->colors[color]);
#else
...
void
SetLineWidth(width, color)
     unsigned int width;
     int color;
{
#ifdef MSW
  TreeDrawingAreaDB->LineWidth = width;
#else
...
static void
DrawNodeHelper (node, pos, parent_pos)
     Tree     *node;
     Point    *pos;
     Point    *parent_pos;
{
#ifdef MSW
  HDC hdc = TreeDrawingAreaDB->hdc;
  RECT rcNode;

  rcNode.left = pos->x;
  rcNode.top = pos->y;
  rcNode.right = pos->x + node->width;
  rcNode.bottom = pos->y + node->height;

  if (node->highlight)
    {
      SetDrawColor(HIGHLIGHT_COLOR);
      SetBkColor (hdc, TreeDrawingAreaDB->colors [HIGHLIGHT_BK_COLOR]);
    }
  else
    {
      SetDrawColor(TREE_COLOR);
      SetBkColor (hdc, TreeDrawingAreaDB->colors [BACKGROUND_COLOR]);
    }
      
  ExtTextOut (hdc,
	      pos->x + node->label.xoffset,
	      pos->y + node->label.yoffset,
	      ETO_OPAQUE, &rcNode,
	      node->label.text, node->label.len, NULL);

  if (node->elision)
    {
      POINT pt[3];
      pt[0].x = pos->x + node->width - ELISION_WIDTH;
      pt[0].y = pos->y + 2;
      pt[1].x = pos->x + node->width - 2;
      pt[1].y = pos->y + node->height / 2;
      pt[2].x = pos->x + node->width - ELISION_WIDTH;
      pt[2].y = pos->y + node->height - 2;
      RefreshPen ();
      SelectNewDeleteOld (CreateSolidBrush (GetTextColor (hdc)));
      Polygon (hdc, pt, 3);
    }
  ...
  if (!node->highlight)
    {
      SelectNewDeleteOld (GetStockObject (HOLLOW_BRUSH));
      Rectangle (hdc, rcNode.left, rcNode.top, rcNode.right, rcNode.bottom);
    }

  if (node->focus)
    DrawFocusRect (hdc, &rcNode);
  
  if (node->parent)
    {
      MoveToEx (hdc, pos->x - 1, pos->y + (node->height / 2), NULL);
      LineTo (hdc, parent_pos->x + node->parent->width + 1,
	      parent_pos->y + (node->parent->height / 2));
    }
#else 
  Widget   w;
  GC       gc;
  ...
  if (node->parent) 
    XDrawLine(XtDisplay(w), XtWindow(w), gc,
	      pos->x - 1,
	      pos->y + (node->height / 2),
	      parent_pos->x + node->parent->width + 1,
	      parent_pos->y + (node->parent->height / 2));
  if (node->elision) {
    XSetFillStyle(TreeDrawingAreaDB->display, TreeDrawingAreaDB->gc,
		  FillTiled);
    XFillRectangle(XtDisplay(w), XtWindow(w), gc,
		   pos->x + node->width - ELISION_WIDTH,
		   pos->y + 1, ELISION_WIDTH, node->height - 1);
    XSetFillStyle(TreeDrawingAreaDB->display, TreeDrawingAreaDB->gc,
		  FillSolid);
  }
#endif
...
{
  if (pos_mode == Old)
    DrawNodeHelper (node, &node->old_pos, node->parent ? &node->parent->old_pos : NULL);
  else
    DrawNodeHelper (node, &node->pos, node->parent ? &node->parent->pos : NULL);
}
     ...
     int recursive;
{
  ...
  if ((select && tree->show_contour) || !select) {
    ...
    SetDrawColor(color);
    SetLineWidth(TreeContourWidth, color);
...
#ifdef MSW
    RefreshPen ();
    MoveToEx (TreeDrawingAreaDB->hdc, x, y, NULL);
#endif
    while (contour) {
      x += contour->dx;
      y += contour->dy;
#ifdef MSW
      LineTo (TreeDrawingAreaDB->hdc, x, y);
#else
      XDrawLine(XtDisplay(w), XtWindow(w), TreeDrawingAreaDB->gc,
		x - contour->dx, y - contour->dy, x, y);
#endif
...
#ifdef MSW
    SelectNewDeleteOld (CreateSolidBrush (GetBkColor (TreeDrawingAreaDB->hdc)));
    Rectangle (TreeDrawingAreaDB->hdc, node->pos.x + node->width - ELISION_WIDTH,
	       node->pos.y + 1, node->pos.x + node->width, node->pos.y + node->height);
#else
