...
It is derived from the etags.c program included with (but not considered
part of) GNU Emacs. 
 ...
 *
 *      Modified for use as ootags.c by Bob Weiner and Steve Baur, BeOpen.com
 *
...
#ifdef OO_BROWSER
  puts ("-O, --oo-browser\n\
	Generate a specialized tags format used only by the BeOpen.com OO-Browser.");
#endif
        ...
        else
          {
            *is_func_or_var = FALSE;
            ...
            /* Handle possible whitespace between macro tag and opening
               ...
               -- Bob Weiner, BeOpen.com, 11/19/1997 */
            while (*p && isspace(*p)) p++;
            if (*p) c = *p;
			      ...
			      tok.named
                                = (structdef == stagseen
                                   ...
                                   /* Also name #define constants,
                                      ...
                                      their #names.
                                      -- Bob Weiner, BeOpen.com, 4/25/1998 */
	...
	case '}':
	  ...
	  if (cblev == 0)
		...
		free (structtag); */
		 ...
		 functions. -- Bob Weiner, BeOpen.com, 11/19/1997 */
...
#else
		{
		  tok.named = TRUE;
                  ...
                  /* ootags categorizes each tag found whereas etags doesn't.
                     ...
                     -- Steve Baur, BeOpen.com, 5/7/1998 */
