...
;;
;; ORIG-DATE:    04-Feb-89
;; LAST-MOD:      7-Aug-99 at 22:51:25 by Bob Weiner
;;
...
;;  19, or InfoDock), NEXTSTEP, Apollo's DM, and you want to use the
...
;;  are in the Emacs "src" directory: for X "x*.c", and for Apollo DM
;;  "apollo.c" and "apollo.el".
;;
...
(defun action-key-error ()
  (error "(Action Key): No action defined for this context; try another location."))
...
(defvar hkey-alist
  '(
    ...
    ((and (eq major-mode 'python-mode) buffer-file-name
	  (smart-c-at-tag-p)) .
     ((python-to-definition) . (python-to-definition)))
    ;;
...
(defalias 'smart-wrolo-assist 'smart-wrolo)
